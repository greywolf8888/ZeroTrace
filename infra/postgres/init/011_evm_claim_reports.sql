...
CREATE TABLE IF NOT EXISTS evm_claim_reports (
  id text PRIMARY KEY CHECK (id ~ '^ecr_[0-9a-f]{24}$'),
  ...
  snapshot_block numeric(30, 0) NOT NULL CHECK (snapshot_block >= to_block),
  ...
  CONSTRAINT evm_claim_reports_result_hash_key UNIQUE (result_hash)
);

CREATE INDEX IF NOT EXISTS evm_claim_reports_subject_latest_idx
  ON evm_claim_reports (
    chain_id,
    token_address,
    subject_address,
    snapshot_block DESC,
...
CREATE OR REPLACE FUNCTION validate_evm_claim_report_insert()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  IF NEW.report ->> 'tokenAddress' IS DISTINCT FROM NEW.token_address
    OR NEW.report ->> 'address' IS DISTINCT FROM NEW.subject_address
    ...
    OR NEW.report #>> '{metadata,snapshot,blockTimestamp}' IS NULL
    ...
    OR jsonb_typeof(NEW.report #> '{custody,evidenceIds}') IS DISTINCT FROM 'array'
    OR jsonb_typeof(NEW.report #> '{custodyMetadata,evidenceIds}') IS DISTINCT FROM 'array'
    OR jsonb_typeof(NEW.report #> '{flow,metadata,evidenceIds}') IS DISTINCT FROM 'array'
    OR jsonb_typeof(NEW.report #> '{flow,inflow,evidenceIds}') IS DISTINCT FROM 'array'
    OR jsonb_typeof(NEW.report #> '{flow,outflow,evidenceIds}') IS DISTINCT FROM 'array'
    OR jsonb_typeof(NEW.report #> '{flow,topCounterparties}') IS DISTINCT FROM 'array'
    OR EXISTS (
      SELECT 1
      FROM jsonb_array_elements(NEW.report #> '{flow,topCounterparties}') counterparty
      WHERE jsonb_typeof(counterparty -> 'evidenceIds') IS DISTINCT FROM 'array'
    )
  THEN
    RAISE EXCEPTION 'EVM Claim Report result conflicts with its stored identity';
  END IF;
  ...
  THEN
    RAISE EXCEPTION 'EVM Claim Report provenance arrays must be canonical';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'EVM Claim Report references missing Evidence';
  END IF;

  IF EXISTS (
    SELECT 1
    FROM (
      SELECT jsonb_array_elements_text(NEW.report #> '{custody,evidenceIds}') AS id
      UNION ALL
      SELECT jsonb_array_elements_text(NEW.report #> '{custodyMetadata,evidenceIds}') AS id
      UNION ALL
      SELECT jsonb_array_elements_text(NEW.report #> '{flow,metadata,evidenceIds}') AS id
      UNION ALL
      SELECT jsonb_array_elements_text(NEW.report #> '{flow,inflow,evidenceIds}') AS id
      UNION ALL
      SELECT jsonb_array_elements_text(NEW.report #> '{flow,outflow,evidenceIds}') AS id
      UNION ALL
      SELECT jsonb_array_elements_text(counterparty -> 'evidenceIds') AS id
      FROM jsonb_array_elements(NEW.report #> '{flow,topCounterparties}') counterparty
    ) nested
    WHERE NOT nested.id = ANY(NEW.evidence_ids)
  ) THEN
    RAISE EXCEPTION 'EVM Claim Report nested Evidence is missing from its provenance';
  END IF;

  SELECT * INTO terminal FROM evidence WHERE id = NEW.terminal_evidence_id;
  expected_locator := 'evm-claim-address:' || NEW.token_address || ':' ||
    NEW.subject_address || ':' || NEW.from_block::text || '-' || NEW.to_block::text;
  IF NOT FOUND
  ...
  THEN
    RAISE EXCEPTION 'EVM Claim Report terminal Evidence conflicts with its identity';
  END IF;
  ...
  THEN
    RAISE EXCEPTION 'EVM Claim Report terminal Evidence Snapshot conflicts with its identity';
  END IF;
  ...
  THEN
    RAISE EXCEPTION 'EVM Claim Report terminal Evidence sources conflict with its provenance';
  END IF;
  RETURN NEW;
END
$function$;

DROP TRIGGER IF EXISTS evm_claim_report_insert_guard ON evm_claim_reports;
CREATE TRIGGER evm_claim_report_insert_guard
BEFORE INSERT ON evm_claim_reports
FOR EACH ROW EXECUTE FUNCTION validate_evm_claim_report_insert();

CREATE OR REPLACE FUNCTION reject_evm_claim_report_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
  RAISE EXCEPTION 'evm_claim_reports is immutable; mutation is forbidden';
END
$function$;

DROP TRIGGER IF EXISTS evm_claim_report_update_guard ON evm_claim_reports;
CREATE TRIGGER evm_claim_report_update_guard
BEFORE UPDATE ON evm_claim_reports
FOR EACH ROW EXECUTE FUNCTION reject_evm_claim_report_mutation();

DROP TRIGGER IF EXISTS evm_claim_report_delete_guard ON evm_claim_reports;
CREATE TRIGGER evm_claim_report_delete_guard
BEFORE DELETE ON evm_claim_reports
FOR EACH ROW EXECUTE FUNCTION reject_evm_claim_report_mutation();

INSERT INTO schema_migrations(version)
VALUES ('011_evm_claim_reports')
