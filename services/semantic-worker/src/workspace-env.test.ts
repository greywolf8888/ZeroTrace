...
describe('loadWorkspaceEnv', () => {
  it('discovers a root .env and preserves an explicit process value', () => {
    ...
    const workspace = join(root, 'services', 'semantic-worker');
    mkdirSync(workspace, { recursive: true });
    const rootKey = `ZEROTRACE_ENV_TEST_${process.pid}_SEMANTIC_ROOT`;
    const explicitKey = `ZEROTRACE_ENV_TEST_${process.pid}_SEMANTIC_EXPLICIT`;
