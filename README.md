...
- durable pairwise Entity relationship timelines over persisted reports, with same-position
  revisions, explicit unobserved-position gaps, Knowledge-state-preserving probability deltas,
  immutable terminal Evidence, and provider-free latest/exact replay;
- bounded exact-Snapshot investigation graphs materialized only from those durable timelines, with
...
The query API also returns strictly validated EVM/Bitcoin/Solana blocks and transactions. Solana
legacy/v0 records resolve loaded Address Lookup Table accounts, account access flags, outer/CPI
instruction paths, recorded SOL/SPL balance effects, and official System/SPL Token/Token-2022 core
asset flows with token-account owner mapping, failed-transaction application state, per-flow
Evidence, and exact atomic reconciliation. Successful finalized Solana transaction analyses are
stored as immutable content-addressed reports and can be replayed by latest or exact report ID
without contacting RPC; automatic provider-failure fallback is explicitly marked as replayed and
...
Temporal is opt-in with `docker compose --profile full up --build`. Apache AGE is an optional
derived traversal index: set `AGE_URL` as documented in
[Local Development](docs/DEVELOPMENT.md#optional-apache-age-investigation-projection), then start
the `graph` profile. Immutable PostgreSQL graph reports remain authoritative if AGE is absent.
...
- [x] Normalize Solana legacy/v0 messages, loaded ALT accounts, outer/CPI instructions and recorded SOL/SPL balance effects without fake zeroes
