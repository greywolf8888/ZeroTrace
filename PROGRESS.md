...
| Runnable foundation              | **Yes; host, fresh local durable-store, browser and remote container gates pass**                        |
| Production acceptance            | **No**                                                                                                   |
| Transaction mode                 | **Read-only; signing/broadcast/private-key custody forbidden**                                           |
| Unit tests                       | **570 passing across 92 files**                                                                          |
| Model evaluation tests           | **1 structural Entity Precision/False-Merge gate passing**                                               |
| Integration tests                | **78 environment-free; 112/112 real-storage pass locally and in disposable CI stores**                   |
| Real-browser E2E                 | **36 passing: Chromium desktop and Pixel 7**                                                             |
| Remote CI                        | **Draft PR #20 head `b4035be` passes CI and CodeQL; protected `main` remains `055daf9`**                 |
| Coverage                         | **Local/remote: 82.44% statements / 76.66% branches / 93.29% functions / 83.55% lines**                  |
...
| Unit tests                     | 570 pass                                         | 92 files, including generic declaration capture/replay plus raw-ledger Action compilation, persistence, scheduling and handler execution                                                                                                            |
| Integration tests              | 78 environment-free; 112 local/remote real-store | all PostgreSQL/ClickHouse/MinIO cases pass in clean local and disposable CI stores; declaration replay adds migration health, restart equality and immutable-row checks                                                                             |
...
| Coverage gate                  | Pass                                             | clean local durable run: 682/682 tests, 82.44% statements, 76.66% branches, 93.29% functions and 83.55% lines                                                                                                                                       |
