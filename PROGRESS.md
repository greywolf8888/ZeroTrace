...
| Unit tests                       | **419 passing across 61 files**                                                                                       |
...
| Coverage                         | **Current local: 82.82% statements / 77.08% branches / 91.93% functions / 84.00% lines**                              |
...
- finalized EIP-1898 reads compare exact contract bytecode, the ERC-1167 implementation, all three
  EIP-1967 slots, ERC-173 `owner()`, and registered Safe singleton/owners/threshold state across
  every configured source at one common canonical block;
...
- each result includes a complete 23-domain coverage matrix, so unqueried tax, blacklist,
...
- a live Alchemy plus BNB Chain inspection of FFT at block `115199429`, hash
  `0x8bd43eaa5ec636bf3f9b0caf6f6f642fc9072a69c636c076ea5d5b085f5654d3`, identified an
  ERC-1167 proxy whose fixed implementation is `0x024f18294970b5c76c0691b87f138a0317156422`.
  ERC-173 `owner()` returned the zero address and no direct right was emitted, while 16 of 23
  coverage domains correctly remained Unknown. The immutable report replayed after the API was
  recreated without an Alchemy credential.
...
| EVM control rights     | Independent standard-surface reads plus custom-role/history/controller reconstruction                                     | Alchemy and BNB Chain agreed on FFT ERC-1167 bytecode, implementation, EIP-1967 slots and ERC-173 owner at one finalized Snapshot; custom token controls, recursive implementation authority, historical validity and non-EVM surfaces remain pending                                                                                                                                                                                                                                                                    |
...
| Flap FFT terminal run  | BSC token `0xdcfb441a1f38802820a4e7b4cc8aab37833c7777`: mechanism, entity, market, RV, Evidence and automatic error audit | Transfer/Safe, buy/exit, durable burn promotion, 37-check market/RV reconciliation, four-transition independent-source supply continuity, and the scoped EVM standard control surface passed. FFT is an ERC-1167 proxy with fixed implementation and zero-valued ERC-173 owner at the tested Snapshot; 16 authorization/control domains remain Unknown. Official attribution, recursive implementation authorization, full-history supply proof, fork settlement and entity calibration still gate a terminal conclusion |
...
| Unit tests                     | 419 pass                                      | 61 files across schemas, adapters, claim auditing, data quality, ingestion, storage, workers and API runtime                            |
...
| Coverage gate                  | Pass                                          | current local: 82.82% statements, 77.08% branches, 91.93% functions, 84.00% lines on 479 tests; 23 opt-in durable tests skipped         |
...
| Database bootstrap             | Pass                                          | fresh bootstrap previously passed; current persistent Compose volume was backed up and upgraded non-destructively through migration 012 |
