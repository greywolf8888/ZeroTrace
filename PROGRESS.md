...
| -------------------------------- | ------------------------------------------------------------------------- |
| Terminal architecture completion | **13% estimated**                                                         |
...
| Unit tests                       | **113 passing across 13 files**                                           |
| Integration tests                | **18 passing: 14 API + 4 real PostgreSQL**                                |
| Real-browser E2E                 | **6 passing: Chromium desktop and Pixel 7**                               |
| Remote CI                        | **Pass on immutable main commit `5f94dca`**                               |
| Coverage                         | **92.80% statements / 81.47% branches / 97.55% functions / 93.94% lines** |
| Real-chain validation            | Ethereum, BSC, Bitcoin, and Solana current-state smoke passed             |
| Durable evidence/history         | Evidence/Snapshot graph wired; historical ingestion not implemented       |
...
| Durable ingestion and chain history  | `NOT_IMPLEMENTED`                           | SQD/Core/Geyser backfill, reorg, reconciliation, and writers are absent           |
| Evidence graph                       | `PARTIALLY_IMPLEMENTED`                     | durable nodes/Snapshots/edges work; raw artifacts and historical ingestion absent |
...
| Unit tests                     | 113 pass                     | 13 files across schemas, adapters, evidence, storage, entity, RV, runtime   |
| API integration                | 14 pass                      | health, storage failure, grounding, Evidence, boundaries, metrics           |
| PostgreSQL integration         | 4 pass                       | migrations, idempotency, restart drilldown, append-only/source constraints  |
| Coverage gate                  | Pass                         | 92.80% statements, 81.47% branches, 97.55% functions, 93.94% lines          |
...
| Database bootstrap             | Pass                         | PostgreSQL 001–003/triggers and three ClickHouse tables/constraint          |
| Runtime/browser smoke          | Pass                         | API/web health, proxy, security headers, desktop/mobile render              |
| Public chain smoke             | Pass for current-state scope | Ethereum/BSC/Bitcoin/Solana snapshot-pinned Evidence; archive scope pending |
