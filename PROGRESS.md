...
| Runnable foundation              | **Yes; host gates and remote disposable-store/container gates pass; local Docker is unavailable**        |
| Production acceptance            | **No**                                                                                                   |
| Transaction mode                 | **Read-only; signing/broadcast/private-key custody forbidden**                                           |
| Unit tests                       | **566 passing across 91 files**                                                                          |
| Model evaluation tests           | **1 structural Entity Precision/False-Merge gate passing**                                               |
| Integration tests                | **77 environment-free; 110/110 real-storage pass in remote disposable stores**                           |
| Real-browser E2E                 | **36 passing: Chromium desktop and Pixel 7**                                                             |
| Remote CI                        | **PR #18 code commit `9d608f5` passed CI and CodeQL; protected-main promotion pending**                  |
| Coverage                         | **Remote durable: 82.39% statements / 76.61% branches / 93.24% functions / 83.49% lines**                |
...
| Unit tests                     | 566 pass                                         | 91 files, including generic raw-ledger Action compilation, persistence, capture scheduling and handler execution                                                                                                                                                           |
| Integration tests              | 77 environment-free; 110 remote real-storage     | all PostgreSQL/AGE/ClickHouse/MinIO cases pass in disposable CI stores; locally, 77 environment-free cases passed and 33 durable cases were explicitly skipped because the Docker engine was unavailable                                                                   |
...
| Coverage gate                  | Pass                                             | remote disposable-store run: 676/676 tests, 82.39% statements, 76.61% branches, 93.24% functions and 83.49% lines; local engine-free run passed 642 tests but correctly failed the durable threshold after 33 store cases skipped                                          |
...
| Docker image build/start       | Remote six-target build pass; local inconclusive | PR #18 built API, web, both workers and both initialization images; the local Docker engine was unavailable, so no local container-start result is claimed                                                                                                                 |
| Database bootstrap             | Pass                                             | isolated empty PostgreSQL applied migrations 001-026 in order; Action Semantics v0.2 plus legacy v0.1 Evidence/immutability/restart tests passed and the temporary database was removed                                                                                    |
...
| Remote CI                      | PR #18 code commit `9d608f5` passed              | [CI run 31547375160](https://github.com/greywolf8888/ZeroTrace/actions/runs/31547375160) and [CodeQL run 31547375237](https://github.com/greywolf8888/ZeroTrace/actions/runs/31547375237) passed; protected-main promotion remains pending until this record commit passes |
